...
let SeriesService = class SeriesService {
    ...
    async findAll() {
        ...
        return all
            .filter((s) => s.tipo === client_1.ProdutoTipo.SERIE)
    ...
    async findTitulo(searchTerm) {
        ...
        let filtered = all.filter((s) => s.tipo === client_1.ProdutoTipo.SERIE &&
        ...
        if (!filtered.length && termBusca.length >= 3) {
            ...
            filtered = all.filter((serie) => {
                if (serie.tipo !== client_1.ProdutoTipo.SERIE)
                    return false;
    ...
    async addSerie(data) {
        console.log('[SeriesService] Recebido dados para criar:', JSON.stringify(data));
        return this.prisma.$transaction(async (tx) => {
            const tipoFinal = data.tipo?.toUpperCase() === 'ANIME'
            ...
            console.log('[SeriesService] Objeto criado no banco:', JSON.stringify(created));
    ...
    async deleteSerie(id) {
        ...
        return this.prisma.$transaction(async (tx) => {
            try {
                ...
                console.log(`[SeriesService] Série ID: ${id} deletada com sucesso. Titulo: ${deleted.titulo}`);
